...
const UserController = {
  ...
  verifyEmail: async (req, res) => {
    ...
    sendVerificationEmail(req.query, res); 
    res.send('Email verification initiated. Check your email for further instructions.');
  },
catch (error) {
  ...
  forgotPassword: async (req, res) => {  let error = req.flash("error");
  ...
  ChangePassword: async (req, res) => {
    try {
      ...
      const passwordMatch = await bcrypt.compare(npassword, user.password);
      if (passwordMatch) {
        req.flash("error", "New password should not be the same as the existing one.");
        return res.redirect("/forgot-password");
      }
